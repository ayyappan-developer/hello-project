...
export class AuthorizeGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean | Promise<boolean> | Observable<boolean> {
    const requiredAccess = this.reflector.getAllAndOverride<string[]>('auth_key', [context.getHandler(), context.getClass()]);
    ...
    const hasExactMatch = requiredAccess.some((requiredAccess) => userAccess?.some((userAccess: string) => userAccess === requiredAccess));
